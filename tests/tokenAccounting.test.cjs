...
test('Hazelnut memory uses a compact state record through the actual provider adapter',async(t)=>{
 ...
 assert.match(request.messages[0].content,/current feelings, their cause, and unresolved repair/);
 assert.match(request.messages[0].content,/who owes each unresolved promise or answer/);
 assert.match(request.messages[0].content,/Aim for 220 words total/);
});

test('memory provider receives thought attribution instead of flattening narration into dialogue',async(t)=>{
 const {summarizeConversation}=require('../dist/lib/providers');
 const old=process.env.NVIDIA_API_KEY;process.env.NVIDIA_API_KEY='test-only';
 t.after(()=>{if(old===undefined)delete process.env.NVIDIA_API_KEY;else process.env.NVIDIA_API_KEY=old});
 let request;t.mock.method(global,'fetch',async(url,options)=>{
  request=JSON.parse(options.body);
  return Response.json({choices:[{message:{content:'User said hello; privately admired Mira.'},finish_reason:'stop'}]});
 });
 for(const intelligence of [3,6,8,10]){
  await summarizeConversation({name:'Mira'},'Earlier facts.',[{role:'user',content:'hey stupid *she is looking gorgeous*'}],false,intelligence);
  assert.match(request.messages[0].content,/Never turn an unspoken thought into dialogue or shared character knowledge/);
  assert.match(request.messages[1].content,/ROLEPLAY_INPUT/);
  assert.match(request.messages[1].content,/"kind":"narration","text":"she is looking gorgeous"/);
 }
});
