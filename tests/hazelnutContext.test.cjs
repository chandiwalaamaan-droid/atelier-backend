...
test('compact premium prompt keeps identity, boundaries, memory and one pacing block',()=>{
 ...
 assert.equal(p.split('TIER-LOCKED REPLY ENVELOPE').length-1,1);
...
test('character commitments survive unrelated turns within the existing context budget',()=>{
 const input=history();
 input[1]=row(1,'assistant','I promised to repair your music box.');
 input[2]=row(2,'user','Please never bring snakes into this story.');
 const original=structuredClone(input);
 const r=selectHazelnutContext({history:input,query:'What happens next?',systemTokens:900,targetTokens:2400});
 const entries=JSON.parse(r.memory.slice(r.memory.indexOf('[{"speaker"')));
 assert.ok(entries.some(e=>e.speaker==='assistant'&&e.quote==='I promised to repair your music box.'));
 assert.ok(entries.some(e=>e.speaker==='user'&&e.quote==='Please never bring snakes into this story.'));
 assert.ok(r.diagnostics.estimatedInputTokens<=2400);
 assert.deepEqual(input,original);
 assert.deepEqual(r.messages.slice(-4),input.slice(-4).map(({role,content})=>({role,content})));
});
test('a recent fulfilled promise is kept verbatim after its older attributed commitment',()=>{
 const input=history();
 input[1]=row(1,'assistant','I promised to repair your music box.');
 input[27]=row(27,'assistant','The music box is repaired. I hand it back to you.');
 const r=selectHazelnutContext({history:input,query:'Thank you.',systemTokens:900,targetTokens:2400});
 assert.match(r.memory,/Recent dialogue overrides outdated details/);
 assert.ok(r.messages.some(m=>m.content===input[27].content));
 assert.equal(r.messages.at(-1).content,input.at(-1).content);
});
