...
router.post("/:characterId", asyncHandler(async (req, res) => {
  ...
  const promptCacheKey = `${compactHazelnut ? "compact-v1" : "standard"}:${characterId}:${character.name}:${character.personality}:${character.backstory}:${character.roleplayNotes}:${character.tagline}:${character.memorySummary ?? ""}:${character.examples ?? ""}:${engine?.id ?? "none"}:${explicitMode}:${spiceLevel ?? "none"}:${roleplayStyle ?? "none"}:${sceneDirective ?? "none"}:${voiceNotes ?? "none"}:${timeGapBucket}`;
  ...
  if (!system) {
    system = buildSystemPrompt(character, {
      ...
      deferReplyGuidance: compactHazelnut,
    });
