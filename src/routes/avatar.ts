...
import { generateGeminiImage, isGeminiConfigured } from "../lib/providers/gemini";
...
const geminiBreaker = new ProviderBreaker("Gemini", { cooldownSeconds: 30, timeoutTripThreshold: 2, timeoutCooldownSeconds: 15 }, "GEMINI");
...
export async function processImageGen(data: ImageGenJobData): Promise<ImageGenJobResult> {
  ...
  try {
    ...
    // Promise.any) matters here because Gemini is billed per image while
    // Pollinations/Cloudflare are free — racing them would pay for Gemini
    // on every single request even when a free provider would've
    // succeeded just as fast. The order differs by content rating:
    //
    //  - SFW: Gemini first (best quality/consistency, worth the cost),
    //    then Pollinations, then Cloudflare as a last resort.
    //  - NSFW: Gemini is skipped entirely — its safety filters reliably
    //    block explicit prompts, so trying it first would just add
    //    latency before falling through anyway. Pollinations goes first
    //    (it's the provider actually configured for uncensored output),
    //    then Cloudflare.
    const geminiAttempt: ProviderAttempt = {
      name: "gemini",
      run: () => generateGeminiImage(prompt, timeoutMs),
    ...
    const chain: ProviderAttempt[] = isExplicit
      ? [pollinationsAttempt, cloudflareAttempt]
      : [geminiAttempt, pollinationsAttempt, cloudflareAttempt];

    const errors: string[] = [];

    for (const attempt of chain) {
      if (attempt.name === "gemini" && !isGeminiConfigured()) {
