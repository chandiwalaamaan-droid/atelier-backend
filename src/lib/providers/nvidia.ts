...
// maxTokensForIntelligence (96-320) is sized for a *visible* reply from a
// non-reasoning provider — Groq/SambaNova/minimax write the answer
// directly, so that budget maps straight to reply length. Nemotron 3
// doesn't always honor enable_thinking:false server-side (see
// IS_NEMOTRON_3_FAMILY comment above) and can spend part of that same
// budget on hidden <think> content first. With only 96-320 tokens total,
// even a little leaked reasoning is enough to exhaust it before any
// visible text is written, which is what was producing the frequent
// EmptyResponseError / "empty completion" fallbacks in production —
// worse on explicit content because the model reasons longer about it.
// Add a fixed buffer for this model family only, so hidden thinking has
// room to finish without needing to touch the model or provider order.
// This does NOT make replies longer: the visible text is independently
// capped downstream by getLengthCeiling's sentence/action-count cutoff in
// chat.ts, which trims on the app's own terms regardless of raw token
// budget — this buffer only protects the *start* of the visible reply.
...
export async function streamNvidiaChat(
  ...
  return streamOpenAICompatibleChat(BASE_URL, apiKey, MODEL, withReasoningToggle(messages), onToken, timeoutMs, clientSignal, genParamsExtraBody(params), effectiveMaxTokens(params));
}
...
export async function completeNvidiaChat(
  ...
  return completeOpenAICompatibleChat(BASE_URL, apiKey, MODEL, withReasoningToggle(messages), timeoutMs, genParamsExtraBody(params), effectiveMaxTokens(params));
}
