...
streaming chat endpoint with tier-aware provider routing:
Vanilla/Strawberry use NVIDIA → Groq → SambaNova → Cloudflare → Ollama;
Chocolate uses Groq → NVIDIA → SambaNova → Cloudflare → Ollama; and
Hazelnut uses Groq → SambaNova → Cloudflare → NVIDIA → Ollama.
...
## Character gallery classification

The seed data keeps the explicitly designated adult, non-family characters in `prisma/explicit-characters.json`. The Discover endpoint returns `isExplicit: true` entries only when `?nsfw=1`/`?nsfw=true` is requested, and SFW requests return only `isExplicit: false` entries.
