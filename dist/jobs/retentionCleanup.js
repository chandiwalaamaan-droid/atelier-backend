...
async function sendInactivityWarnings(frontendUrl) {
    ...
    for (const user of candidates) {
        try {
            await (0, mailer_1.sendMail)(user.email, "Your Atelier account will be deleted in 30 days due to inactivity", `<p>Hi ${user.displayName},</p>
         ...
         (this cannot be undone).</p>`, `Hi ${user.displayName}, your Atelier account has been inactive and will be deleted in 30 days ` +
    ...
    for (const user of candidates) {
        try {
            await db_1.prisma.$transaction([
                ...
                db_1.prisma.message.deleteMany({ where: { userId: user.id } }),
                db_1.prisma.character.deleteMany({ where: { ownerId: user.id } }),
                db_1.prisma.report.deleteMany({ where: { reporterId: user.id } }),
                db_1.prisma.passwordResetToken.deleteMany({ where: { userId: user.id } }),
                db_1.prisma.emailVerificationToken.deleteMany({ where: { userId: user.id } }),
