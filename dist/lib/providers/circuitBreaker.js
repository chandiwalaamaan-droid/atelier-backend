...
class ProviderBreaker {
    ...
    recordTimeout() {
        ...
        const shouldTrip = this.consecutiveTimeouts >= this.timeoutTripThreshold && Date.now() >= this.cooldownUntil;
        if (shouldTrip) {
            this.cooldownUntil = Date.now() + this.timeoutCooldownSeconds * 1000;
            this.consecutiveTimeouts = 0;
            console.warn(`[breaker] ${this.name}: ${this.timeoutTripThreshold} consecutive timeouts — opening for ${this.timeoutCooldownSeconds}s`);
        }
    }
    /**
...
exports.ProviderBreaker = ProviderBreaker;
