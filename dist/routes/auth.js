...
function validateNewAccountFields(displayName, birthdateRaw, tosAccepted) {
    if (!displayName)
    ...
    if (calculateAge(birthdate) < MINIMUM_AGE_YEARS)
        return { error: "You must be 18 or older to use Atelier." };
    if (!tosAccepted)
        return { error: "You must accept the Terms of Service and Content Policy to continue." };
    return { birthdate };
}
...
async function issueEmailVerification(userId, email, frontendUrl) {
    const raw = issueRawToken();
    await db_1.prisma.emailVerificationToken.create({
    ...
    await (0, mailer_1.sendMail)(email, "Verify your Atelier email", `<p>Confirm this is your email address to finish setting up your Atelier account.</p><p><a href="${link}">${link}</a></p><p>This link expires in 24 hours.</p>`, `Verify your email: ${link} (expires in 24 hours)`);
}
router.post("/register", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const ip = (0, rateLimit_1.getClientIp)(req);
    const limit = (0, rateLimit_1.checkRateLimit)(`register:${ip}`, 5, 60 * 60);
    if (limit.limited) {
    ...
    const token = await (0, auth_1.createSessionToken)(user.id);
    (0, auth_1.setSessionCookie)(res, token);
    const frontendUrl = (process.env.FRONTEND_URL || "http://localhost:3000").split(",")[0].trim();
    issueEmailVerification(user.id, user.email, frontendUrl).catch((err) => console.error("Failed to send verification email:", err));
    return res.json({
        id: user.id,
        email: user.email,
...
router.post("/google", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const ip = (0, rateLimit_1.getClientIp)(req);
    const limit = (0, rateLimit_1.checkRateLimit)(`google-auth:${ip}`, 20, 60 * 60);
    if (limit.limited) {
        res.set("Retry-After", String(limit.retryAfterSeconds));
...
router.post("/google/complete", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const ip = (0, rateLimit_1.getClientIp)(req);
    const limit = (0, rateLimit_1.checkRateLimit)(`google-complete:${ip}`, 10, 60 * 60);
    if (limit.limited) {
        res.set("Retry-After", String(limit.retryAfterSeconds));
        return res.status(429).json({ error: "Too many attempts. Please try again later." });
    }
        ...
        try {
            user = await db_1.prisma.user.create({
                data: {
                    ...
                    passwordHash: await (0, auth_1.hashPassword)(crypto_1.default.randomBytes(32).toString("hex")),
                    displayName,
...
router.post("/login", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const ip = (0, rateLimit_1.getClientIp)(req);
    const limit = (0, rateLimit_1.checkRateLimit)(`login:${ip}`, 10, 15 * 60);
    if (limit.limited) {
        res.set("Retry-After", String(limit.retryAfterSeconds));
...
router.post("/forgot-password", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const ip = (0, rateLimit_1.getClientIp)(req);
    const limit = (0, rateLimit_1.checkRateLimit)(`forgot-password:${ip}`, 5, 60 * 60);
    if (limit.limited) {
        res.set("Retry-After", String(limit.retryAfterSeconds));
    ...
    if (user) {
        ...
        await (0, mailer_1.sendMail)(user.email, "Reset your Atelier password", `<p>Someone requested a password reset for this account. If that was you, set a new password:</p><p><a href="${link}">${link}</a></p><p>This link expires in 1 hour. If you didn't request this, you can ignore this email.</p>`, `Reset your password: ${link} (expires in 1 hour; ignore if you didn't request this)`).catch((err) => console.error("Failed to send reset email:", err));
    }
    return res.json(genericResponse);
}));
...
router.post("/reset-password", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const ip = (0, rateLimit_1.getClientIp)(req);
    const limit = (0, rateLimit_1.checkRateLimit)(`reset-password:${ip}`, 10, 60 * 60);
    if (limit.limited) {
        res.set("Retry-After", String(limit.retryAfterSeconds));
        return res.status(429).json({ error: "Too many attempts. Please try again later." });
    }
    ...
    if (password.length < 8)
        return res.status(400).json({ error: "Password must be at least 8 characters." });
    const record = await db_1.prisma.passwordResetToken.findUnique({ where: { tokenHash: hashToken(rawToken) } });
    if (!record || record.usedAt || record.expiresAt.getTime() < Date.now()) {
        return res.status(400).json({ error: "That reset link is invalid or has expired. Request a new one." });
    }
    const passwordHash = await (0, auth_1.hashPassword)(password);
    await db_1.prisma.$transaction([
        db_1.prisma.user.update({ where: { id: record.userId }, data: { passwordHash } }),
        db_1.prisma.passwordResetToken.update({ where: { id: record.id }, data: { usedAt: new Date() } }),
    ]);
    return res.json({ ok: true });
}));
...
router.post("/verify-email", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const rawToken = typeof req.body?.token === "string" ? req.body.token : "";
    if (!rawToken)
        return res.status(400).json({ error: "Missing verification token." });
    const record = await db_1.prisma.emailVerificationToken.findUnique({ where: { tokenHash: hashToken(rawToken) } });
    if (!record || record.expiresAt.getTime() < Date.now()) {
        return res.status(400).json({ error: "That verification link is invalid or has expired." });
    }
    await db_1.prisma.$transaction([
        db_1.prisma.user.update({ where: { id: record.userId }, data: { emailVerified: true } }),
        db_1.prisma.emailVerificationToken.deleteMany({ where: { userId: record.userId } }),
    ]);
    return res.json({ ok: true });
}));
...
router.post("/resend-verification", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = await (0, auth_1.getCurrentUserId)(req);
    if (!userId)
        return res.status(401).json({ error: "Not signed in." });
    const limit = (0, rateLimit_1.checkRateLimit)(`resend-verification:${userId}`, 3, 60 * 60);
    if (limit.limited) {
        res.set("Retry-After", String(limit.retryAfterSeconds));
        return res.status(429).json({ error: "Too many requests. Please try again later." });
    }
