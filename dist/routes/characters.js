...
function parseCharacterInput(body) {
...
async function loadOwnedCharacter(id, userId) {
    const character = await db_1.prisma.character.findUnique({ where: { id } });
...
router.get("/", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const characters = await db_1.prisma.character.findMany({
...
async function enforceVerifiedForPublic(userId, isPublic) {
    if (!isPublic)
        return null;
    const user = await db_1.prisma.user.findUnique({ where: { id: userId }, select: { emailVerified: true } });
...
router.post("/", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const character = await db_1.prisma.character.create({
...
router.post("/draft", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = await (0, auth_1.getCurrentUserId)(req);
    if (!userId)
        return res.status(401).json({ error: "Not signed in." });
    const limit = (0, rateLimit_1.checkRateLimit)(`draft:${userId}`, 10, 60);
    if (limit.limited) {
    ...
    try {
        const draft = await (0, providers_1.draftCharacterWithFallback)(idea, allowExplicit);
...
router.post("/import", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!userId)
        return res.status(401).json({ error: "Not signed in." });
    const limit = (0, rateLimit_1.checkRateLimit)(`import:${userId}`, 5, 60);
    if (limit.limited) {
        res.set("Retry-After", String(limit.retryAfterSeconds));
    ...
    for (let i = 0; i < raw.length; i++) {
        ...
        if (!parsed.data) {
            errors.push({ index: i, name: clean(raw[i]?.name), error: parsed.error ?? "Invalid entry." });
            continue;
        }
...
router.get("/discover", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = await (0, auth_1.getCurrentUserId)(req);
    if (!userId)
        return res.status(401).json({ error: "Not signed in." });
...
router.put("/:id", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const character = await db_1.prisma.character.update({
...
router.delete("/:id", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!character)
        return res.status(404).json({ error: "Character not found." });
    await db_1.prisma.character.delete({ where: { id: req.params.id } });
...
router.post("/:id/remix", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = await (0, auth_1.getCurrentUserId)(req);
    if (!userId)
        return res.status(401).json({ error: "Not signed in." });
    const source = await db_1.prisma.character.findUnique({ where: { id: req.params.id } });
