...
const cloudflare_1 = require("../lib/providers/cloudflare");
...
function cacheKey(prompt, width, height) {
...
const upload = (0, multer_1.default)({ storage: multer_1.default.memoryStorage(), limits: { fileSize: MAX_BYTES } });
// POST /api/characters/:id/avatar — upload an image file
router.post("/:id/avatar", upload.single("avatar"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const avatarUrl = await (0, cloudinary_1.uploadAvatarBuffer)(file.buffer, publicId);
    const updated = await db_1.prisma.character.update({ where: { id: req.params.id }, data: { avatarUrl } });
...
function buildImagePrompt(character, customPrompt) {
    if (customPrompt?.trim()) {
        return customPrompt.trim().slice(0, 4000);
    }
    const base = `A highly polished digital portrait of a fictional character named ${character.name}. ` +
    ...
    if (character.isExplicit) {
        return (`${base} Mature, alluring adult energy with tasteful sensuality, confident body language, and flattering intimate styling. ` +
...
async function generatePollinationsImage(character, prompt, timeoutMs) {
...
async function withPollinationsRetry(fn, maxAttempts = 2) {
    let lastErr;
...
async function isBlankOrBlockedImage(bytes) {
    try {
        // Downscale first — stats() on a huge image is wasteful, and we only
        // need a coarse signal, not per-pixel precision.
        const { channels } = await (0, sharp_1.default)(bytes).resize(32, 32, { fit: "fill" }).stats();
...
// ---------------------------------------------------------------------------
async function generateAvatarImage(character, prompt, timeoutMs) {
    const errors = [];
    const t0 = Date.now();
    // Try Pollinations first (free, keyless, uncensored, FLUX model)
    const pollStart = Date.now();
    try {
        const bytes = await withPollinationsRetry(() => generatePollinationsImage(character, prompt, timeoutMs));
        ...
        const msg = err instanceof Error ? err.message : String(err);
        console.warn(`[avatar] Pollinations failed after ${Date.now() - pollStart}ms, falling back to Cloudflare:`, msg);
        errors.push(`pollinations: ${msg}`);
    }
    // Final fallback: Cloudflare Workers AI
    if ((0, cloudflare_1.isCloudflareConfigured)()) {
        const cfStart = Date.now();
        try {
            const bytes = await (0, cloudflare_1.generateCloudflareImage)(prompt, timeoutMs);
            if (await isBlankOrBlockedImage(bytes)) {
                throw new Error("returned a blank/blocked image (likely safety filter)");
            }
...
function buildSceneImagePrompt(character) {
    const base = `Cinematic scene featuring ${character.name}. ` +
        `Tagline: ${character.tagline || "n/a"}. Traits: ${character.personality}. ` +
        `Dramatic lighting, rich atmosphere, highly detailed, photorealistic or stylized, 8k.`;
    if (character.isExplicit) {
        return (`${base} Mature adult scene, sensual atmosphere, intimate moment, tasteful but uninhibited. ` +
...
async function generateSceneImage(prompt, width, height, isExplicit, timeoutMs) {
    const errors = [];
    const t0 = Date.now();
    // Try Pollinations first (exact width/height, keyless, FLUX model)
    const pollStart = Date.now();
    try {
        const bytes = await withPollinationsRetry(() => generatePollinationsSceneImage(prompt, width, height, isExplicit, timeoutMs));
        if (await isBlankOrBlockedImage(bytes)) {
            throw new Error("returned a blank/blocked image (likely safety filter)");
        }
...
async function generateHuggingFaceImageWithSize(prompt, width, height, timeoutMs) {
    ...
    if (!res.ok) {
        ...
        throw new Error(`Hugging Face API error ${res.status} [model=${model}, url=${url}]: ${errText.slice(0, 300)}`);
    }
    const contentType = res.headers.get("content-type") || "";
    if (!contentType.startsWith("image/")) {
        const errText = await res.text().catch(() => "");
        throw new Error(`Hugging Face returned non-image response [model=${model}]: ${errText.slice(0, 300)}`);
    }
...
async function generatePollinationsSceneImage(prompt, width, height, isExplicit, timeoutMs) {
    const params = new URLSearchParams({
        width: String(width),
...
router.post("/:id/image/generate", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = await (0, auth_1.getCurrentUserId)(req);
    if (!userId)
        return res.status(401).json({ error: "Not signed in." });
    const character = await db_1.prisma.character.findUnique({ where: { id: req.params.id } });
    ...
    const limit = (0, rateLimit_1.checkRateLimit)(`image:${userId}`, 10, 60);
    if (limit.limited) {
    ...
    if (cached) {
        const cleanBytes = await (0, sharp_1.default)(cached).toBuffer().catch(() => cached);
        const publicId = `${req.params.id}-${Date.now()}-scene`;
        const imageUrl = await (0, cloudinary_1.uploadAvatarBuffer)(cleanBytes, publicId);
    ...
    const cleanBytes = await (0, sharp_1.default)(result.bytes).toBuffer().catch(() => result.bytes);
    const publicId = `${req.params.id}-${Date.now()}-scene`;
    const imageUrl = await (0, cloudinary_1.uploadAvatarBuffer)(cleanBytes, publicId);
...
router.post("/:id/avatar/generate", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = await (0, auth_1.getCurrentUserId)(req);
    if (!userId)
        return res.status(401).json({ error: "Not signed in." });
    const character = await db_1.prisma.character.findUnique({ where: { id: req.params.id } });
    ...
    if (cached) {
        ...
        const avatarUrl = await (0, cloudinary_1.uploadAvatarBuffer)(cleanBytes, publicId);
        const updated = await db_1.prisma.character.update({ where: { id: req.params.id }, data: { avatarUrl } });
...
function buildBackgroundPrompt(character) {
    const base = `A stunning atmospheric background scene for a character named ${character.name}. ` +
        `Tagline: ${character.tagline || "n/a"}. Traits: ${character.personality}. ` +
    ...
    if (character.isExplicit) {
        return (`${base} Mature, sensual ambient atmosphere with warm mood lighting, velvety textures, ` +
            `intimate setting, elegant and tasteful. No explicit nudity, but a seductive, luxurious ambiance.`);
    }
    return (`${base} Beautiful, immersive environment with a sense of wonder and emotional depth. ` +
...
router.post("/:id/background/generate", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = await (0, auth_1.getCurrentUserId)(req);
    if (!userId)
        return res.status(401).json({ error: "Not signed in." });
    const character = await db_1.prisma.character.findUnique({ where: { id: req.params.id } });
    ...
    const timeoutMs = Number(process.env.IMAGE_GEN_TIMEOUT_SECONDS || "18") * 1000;
    // Rate limit
    const limit = (0, rateLimit_1.checkRateLimit)(`background:${userId}`, 10, 60);
    if (limit.limited) {
        res.set("Retry-After", String(limit.retryAfterSeconds));
        return res.status(429).json({ error: "Too many image requests. Please slow down a bit." });
    }
    ...
    const backgroundUrl = await (0, cloudinary_1.uploadAvatarBuffer)(cleanBytes, publicId);
    const updated = await db_1.prisma.character.update({ where: { id: req.params.id }, data: { backgroundUrl } });
    console.log(`[background] generated via ${result.provider}`);
    return res.json({ character: updated });
}));
// POST /api/characters/:id/background — upload a custom background image
router.post("/:id/background", upload.single("background"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = await (0, auth_1.getCurrentUserId)(req);
    if (!userId)
        return res.status(401).json({ error: "Not signed in." });
    const character = await db_1.prisma.character.findUnique({ where: { id: req.params.id } });
    ...
    const publicId = `${req.params.id}-${Date.now()}-bg`;
    const backgroundUrl = await (0, cloudinary_1.uploadAvatarBuffer)(file.buffer, publicId);
    const updated = await db_1.prisma.character.update({ where: { id: req.params.id }, data: { backgroundUrl } });
    return res.json({ character: updated });
}));
// DELETE /api/characters/:id/background — remove the background image
router.delete("/:id/background", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = await (0, auth_1.getCurrentUserId)(req);
    if (!userId)
        return res.status(401).json({ error: "Not signed in." });
    const character = await db_1.prisma.character.findUnique({ where: { id: req.params.id } });
